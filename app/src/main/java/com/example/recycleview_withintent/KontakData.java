...
public class KontakData {
    private static String[] namaKontak = {
            "Ahmad Dahlan",
            "Ahmad Yani",
            "Sutomo",
            "Gatot Soebroto",
    ...
    private static String[] detailKontak = {
    ...
    private static int[] imageKontak = {
    ...
    public static ArrayList<KontakModel> getKontakList(){
        KontakModel kontakModel = null;
        ArrayList<KontakModel> list = new ArrayList<>();

        for (int i = 0; i < namaKontak.length; i++){
            kontakModel = new KontakModel();
            kontakModel.setNama(namaKontak[i]);
            kontakModel.setDes(detailKontak[i]);
            kontakModel.setThumb(imageKontak[i]);

            list.add(kontakModel);
        }
        return list;
    }
}
